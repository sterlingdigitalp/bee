...
import { Check, Mic, Settings, X } from "lucide-react";
...
export default function Widget(){
  ...
  return <div className={`widget ${state}`} onDoubleClick={toggle} {...(!locked?{"data-tauri-drag-region":true}:{})}>
    <button className="widget-logo" onClick={toggle} aria-label={state==="listening"?"Stop recording":"Start recording"}><Mic size={15}/></button>
    {state==="idle"&&<><span className="widget-title">BridgeVoice</span><kbd>{shortcut}</kbd></>}
    ...
    {state==="error"&&<><span className="widget-error">!</span><span className="widget-title">Check BridgeVoice</span></>}
